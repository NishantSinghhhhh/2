...
- [ ] R.10: The configuration follows the [preCICE Adapter and Tooling Configuration Schema](https://github.com/precice/preeco-orga/blob/main/adapter-config-schema), either directly in JSON or YAML or indirectly via auto-conversion. The adapter supports loading the file from the standard path `./precice-adapter-config.json` (example for JSON). Validating a JSON configuration against a schema can be done with the Python library [jsonschema](https://pypi.org/project/jsonschema/) or via the [JTutor GUI](https://validationproofs.oa.r.appspot.com/), for example.
...
- [ ] R.12: Any flux-related fields have a configurable direction (following the schema, see R.10) and are assumed to be facing outwards when written, and inwards when read by default.
