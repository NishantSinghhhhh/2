...
<br>
{{- $dev_leads := .Site.Data.developer.leads  | sort "name" -}}
<div class="row">
<div class="col-md-8 offset-md-2">
<ul class="devlist">
  {{- range $loop, $p := dev_leads -}}
  <li{{- if $loop.IsFirst -}} class="devlist-first"{{- end -}}>
    ...
    <ul class="devlist-right">
      {{- if p.url -}}<li><a href="{{ p.url }}" alt="See the institutional website" class="no-icon"><i class="fas fa-university"></i></a></li>{{- end -}}
...
<div class="row">
{{- range $loop, $item := .Site.Data.funding -}}
<div class="col-md-6">
        <div class="panel panel-default panel-custom">
          ...
          <div class="panel-body">
            <div class="row">
              ...
              <div class="col-sm-8">
                {{- if item.description -}}<p><small>{{ item.description }}</small></p>{{- end -}}
                {{- if item.number -}}<p>Funding number: {{ item.number }}</p>{{- end -}}
