...
{{- range $loop, $pub := .Site.Data.publications -}}
...
{{- $years := .Site.Data.publications  | group "year"  | sort "title" -}}
{% for year in years reversed %}

<div class="row">
{{- range $loop, $pub := year.items -}}
{{- if in $pub.tag "fenics" -}}
<div class="col-md-10 offset-md-1">
  <div class="panel panel-primary panel-precice">
    <div class="panel-heading-precice">
      <strong>{{ pub.title }}</strong>
    ...
    <div class="panel-body">
      <p>
        <em>{{ pub.authors }}</em>
        {{- if pub.journal.name -}}{{ pub.journal.name }}, {{- end -}}
        {{- if pub.journal.volume -}}Volume {{ pub.journal.volume }}, {{- end -}}
        {{- if pub.journal.publisher -}}{{ pub.journal.publisher }}, {{- end -}}
        {{ pub.year }}{{- if pub.doi -}}, <a href="https://www.doi.org/{{pub.doi}}">doi:{{pub.doi}}</a>{{- end -}}.
      </p>
      {{- if pub.pub-url -}}
      <a href="{{pub.pub-url}}">Publisher's site</a>&nbsp;&nbsp;
      {{- end -}}
